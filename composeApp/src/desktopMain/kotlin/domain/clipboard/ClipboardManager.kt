...
actual class ClipboardManager actual constructor() {
     ...
     * @return 操作结果
     */
    actual fun dumpClipboardToFile(savePath: String): ClipboardResult {
        return try {
            // 确保目标目录存在
            ...
            // 获取所有剪贴板格式
            val formats = getAllClipboardFormats()
            if (formats.isEmpty()) {
                return ClipboardResult.Error("剪贴板为空，没有可保存的内容")
            }

            // 存储所有格式的数据
            val clipboardDataList = mutableListOf<ClipboardFormatData>()
            ...
            try {
                for (formatId in formats) {
                    try {
                        // 获取剪贴板数据
                        val hData = user32.GetClipboardData(formatId)
                        if (hData != null) {
                            // 获取格式名称
                            val formatName = getClipboardFormatName(formatId)

                            // 获取数据大小
                            val size = kernel32.GlobalSize(hData)
                            if (size > 0) {
                                // 锁定内存并读取数据
                                val lockedData = kernel32.GlobalLock(hData)
                                if (lockedData != null) {
                                    try {
                                        val data = ByteArray(size)
                                        lockedData.read(0, data, 0, size)

                                        clipboardDataList.add(
                                            ClipboardFormatData(
                                                formatId = formatId,
                        ...
                        // 单个格式读取失败，继续处理其他格式
                        println("格式 $formatId 读取失败: ${e.message}")
                    }
                }
            } finally {
            ...
            if (clipboardDataList.isEmpty()) {
                return ClipboardResult.Error("未能读取任何剪贴板数据")
            }

            // 使用 Java 序列化保存数据
            ObjectOutputStream(FileOutputStream(saveFile)).use { oos ->
                oos.writeObject(clipboardDataList.map { formatData ->
                    SerializableClipboardData(
                        formatId = formatData.formatId,
                        formatName = formatData.formatName,
                        data = formatData.data
                    )
                })
            }

            ClipboardResult.Success("成功保存 ${clipboardDataList.size} 种格式到: $savePath")
        } catch (e: Exception) {
            ClipboardResult.Error("保存失败: ${e.message}")
        }
    }
     ...
     * @return 操作结果
     */
    actual fun copyWithReplacementsViaFile(
        sourcePath: String,
        targetPath: String,
        replacements: Map<String, String>
    ): ClipboardResult {
        return try {
            val sourceFile = File(sourcePath)
            if (!sourceFile.exists()) {
                return ClipboardResult.Error("源文件不存在: $sourcePath")
            }
            ...
            if (clipboardDataList.isEmpty()) {
                return ClipboardResult.Error("源文件中没有可处理的数据")
            }

            // 2. 对数据进行变量替换
            val modifiedDataList = clipboardDataList.map { formatData ->
                var data = formatData.data
            ...
            // 3. 将修改后的数据写入目标文件
            ObjectOutputStream(FileOutputStream(targetFile)).use { oos ->
                oos.writeObject(modifiedDataList)
            }

            println("已将填充后的数据写入: $targetPath")

            // 4. 从目标文件读取并写入剪贴板（不再进行替换）
            restoreClipboardFromFile(targetPath, emptyMap())
        } catch (e: Exception) {
            ClipboardResult.Error("复制失败: ${e.message}")
        }
    }
     ...
     * @return 格式 ID 列表
     */
    private fun getAllClipboardFormats(): List<Int> {
        ...
        try {
            var currentFormat = 0
            while (true) {
                currentFormat = user32.EnumClipboardFormats(currentFormat)
     ...
     * @return 格式名称，如果是标准格式则返回 "Format_{id}"
     */
    private fun getClipboardFormatName(formatId: Int): String {
        return try {
            val buffer = CharArray(256)
            val length = user32.GetClipboardFormatNameW(formatId, buffer, buffer.size)
            if (length > 0) {
